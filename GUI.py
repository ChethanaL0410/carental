...
def Admin_Opt():
		...
		addbutton=Button(opt_menu,command=add_book,text=" Add Cars ",bg='light pink',height=1,width=12,font=k_font)
		delbutton=Button(opt_menu,command=del_book,text=" Remove Cars ",bg='light blue',height=1,width=12,font=k_font)
...
def add_book():
	...
	book_id_label=Label(add_menu,text="Car Number (Should be 5 digit)")
	book_label=Label(add_menu,text="Company")
	author_label=Label(add_menu,text="Model")
	book_id=Entry(add_menu)
	book_name=Entry(add_menu)
	author_name=Entry(add_menu)
	addbutton1=Button(add_menu,command=add_check,text=" Add Car ",bg='dark orange',height=1,width=10,font=k_font)
...
def add_check():
	...
	if len(b_name)==0:
		tkinter.messagebox.showinfo("Add Car","You did not type a Car name O_O")
		add_menu.lift()
		return(add_book)
...
def del_book():
	...
	delbutton1=Button(del_menu,command=del_check,text=" Remove Car ",bg='dark orange',height=1,width=10,font=k_font)
	...
	borrow_label=Label(del_menu,text="Car Number")
	...
	with open("Bindex.txt",'r+') as file:
		...
		while line:
			...
			if words[0]==del_id:
				...
				index=file.tell()
				line=file.readline()
	tkinter.messagebox.showinfo("Delete","Car Successfully removed")
	del_menu.destroy()
...
def Main_Menu():
	...
	orionLabel=Label(base, text="CAR RENTAL",bg='dark orange',font=("Castellar", "50","bold","italic","underline"),fg="black")
	welcomeLabel=Label(base,text=("Welcome!  "+id),font=("Times new roman","50","bold"))
	imageLibrary = PhotoImage(file="Sedan-Car-Rental-Bangalore.gif")
	topFrame=Frame(base)
	...
	#Buttons
	borrow_but=Button(bottomFrame,bg="black",fg="white",text="Rent Cars",font=in_font,height=5,width=15,command=borrow_in)
	return_but=Button(bottomFrame,bg="dark orange",text="Return cars",font=in_font,height=5,width=15,command=return_in)
	search_but=Button(bottomFrame,bg="black",fg="white",text="Search for cars",font=in_font,height=5,width=15,command=search_in)
...
def borrow_in():
	...
	borrow_label1=Label(borrow_menu,text="<<< Please enter the Number that you wish to borrow >>>",font=("Times", "20","bold","italic"),bg="light blue")
	borrow_label=Label(borrow_menu,text="Car number")
...
def borrow_check():
	...
	for l in f:
		...
		if l[0] ==  id:
			count += 1
	if count >= 3:
		tkinter.messagebox.showinfo("Borrow", "Cannot borrow more than 3 Cars")
		borrow_menu.destroy()
		...
		if len(bbook) == 0:
			tkinter.messagebox.showinfo("Rent","You did not type anything O_O")
			borrow_menu.lift()
			return(borrow_in)

		pos = binary_search('Bindex.txt', bbook)
		if pos == -1:
			tkinter.messagebox.showinfo("Rent","The book that you had entered is not in our database,sorry,please enter a different book")
			borrow_menu.lift()
		else:
			f2 = open('BData.txt', 'r+')
			f2.seek(pos)
			...
			if(w2[3] == 'Y'):
				...
				f2.write(l3)
				f2.close()
				tkinter.messagebox.showinfo("Rent","The Car you have selected has been successfully borrowed. Please return it by:" +'\n'+ str(enddate) )
				...
				f3.close()
				key_sort('Record.txt')
				Done2=tkinter.messagebox.askyesno("Rent","Do you want to borrow another car?")
				if Done2==True:
					borrow_menu.destroy()
					borrow_in()
				else:
				...
				tkinter.messagebox.showinfo("Rent","This book is currently unavailable, please select another car")
				borrow_menu.lift()
...
def return_in():
	...
	return_label=Label(return_menu,text="Car Number")
	...
	return_label1=Label(return_menu,text=" Please enter the Car Number that you wish to return ",font=("Times", "12","bold","italic"),bg="light blue")
...
def search_check():
	...
	if (pos == -1):
		...
		search_result.insert(1,"Car Number :" + b_id)
		search_result.insert(2,"Company:" + book)
